...
**`terraform plan` with dummy AWS credentials:** Initially BLOCKED by AWS provider v6.51.0 behavior change. Provider v6.51.0 unconditionally calls `STS:GetCallerIdentity` during provider initialization, regardless of whether any data sources or state refresh is needed. The dummy credentials (`AWS_ACCESS_KEY_ID=dummy`) reach real AWS STS and receive `HTTP 403 InvalidClientTokenId`. This is a regression from provider ~> 5.x behavior. Per D-07, `skip_credentials_validation` cannot be added to `providers.tf` (kept clean for real applies).

> **RESOLVED (orchestrator gap-fix, commit `448f65a`).** The blocker above was resolved without violating D-07. A dedicated `make plan-check` target now runs the offline gate by writing a transient `gate_override.tf` (local backend + stubbed AWS provider with the `skip_*` flags) into `envs/prod`, running `terraform fmt -check`, `terraform validate`, and `terraform plan`, then removing the override on an EXIT trap. `providers.tf`/`backend.tf` stay clean and `make plan` was restored to the real S3/AWS path. **Live result: `terraform plan` → `Plan: 9 to add, 0 to change, 0 to destroy`** — VPC, 2 public subnets, IGW, public route table + 2 associations, ALB SG, task SG; no NAT gateway; four outputs present. The phase's "non-empty plan" core value is now met by a real plan run, not just grep fallback. Terraform 1.13.1 was installed to `~/.local/bin` to enable the gate.
